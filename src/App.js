                ...
                {loading ? (
                  ...
                  filteredData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-indigo-50/40 transition-colors group">
                      ...
                      {activeTab === 'custgroup' && (<><td className="px-3 py-0.5 font-bold text-slate-800 text-[14px] truncate">{item.group_name}</td><td className="px-3 py-0.5 text-slate-600 font-mono text-[13px]">{item.group_code}</td><td className="px-3 py-0.5 text-slate-600 bg-amber-50 font-mono text-[13px]">{item.chanel_code}</td><td className="px-3 py-0.5 text-slate-500 font-medium text-[13px] truncate">{item.group_cha}</td><td className="px-3 py-0.5"><span className="px-1.5 py-0 bg-amber-50 text-amber-700 rounded text-[12px] font-bold border border-amber-100">{item.limit_code}</span></td></>)}
